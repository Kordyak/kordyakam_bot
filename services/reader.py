...
class Reader:
    ...
    def __init__(self, book_path, user_id):
        ...
        self.book_mtime = book_path.stat().st_mtime

        cache = BookCache.get_cache(book_path)
        self.all_paragraphs = cache["paragraphs"]
        self.book_title, self.book_author, self.description, self.cover = cache["metadata"]
        self.mtime = cache["mtime"]

        self.index = self.load_state()
        self.index_all = len(self.all_paragraphs)
    ...
    # STATE
    def load_state(self):
        return UserState.get_index(self.user_id)

    def save_state(self, idx):
        UserState.save_index(self.user_id, idx)
    ...
    def get_next_chunk(self, min_len=300):
        ...
        while self.index < self.index_all:
            paragraph = self.all_paragraphs[self.index]
